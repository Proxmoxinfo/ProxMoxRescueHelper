...
# - GitHub: https://github.com/yourgithubrepo
...
clear_list() {    
    ...
    echo "SSH key cache cleared for localhost port 2222."
    printf "quit\n" | nc 127.0.0.1 4444
...
run_qemu() {
    ...
    if [ "$USE_UEFI" == "true" ]; then
        QEMU_COMMON_ARGS="-daemonize -enable-kvm -m 10240 -bios /usr/share/ovmf/OVMF.fd -vnc :0,password=on -monitor telnet:127.0.0.1:4444,server,nowait"
    else
        QEMU_COMMON_ARGS="-daemonize -enable-kvm -m 10240 -vnc :0,password=on -monitor telnet:127.0.0.1:4444,server,nowait"
    fi
    if [ "$task" == "install" ]; then
        ...
        ./noVNC/utils/novnc_proxy --vnc localhost:5900 --listen $IP_ADDRESS:$NOVNC_PORT > /dev/null 2>&1 &
        NOVNC_PID=$!
        while true; do
...
# Первоначальный вызов функции меню при старте скрипта
print_logo
show_menu
